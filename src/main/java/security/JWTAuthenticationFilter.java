...
//Intercept the request before it reaches our endpoint-code (which we can do with a filter).
 @Override   
...
//If token is not valid generate an error response (403)
     try {
       UserPrincipal user = getUserPrincipalFromTokenIfValid(token);
       //What if the client had logged out????
//If request was authenticated, generate a Principal and a SecurityContext instance to be user later in the request chain (when our rest endpoint code is called) for authorization
...
//Check whether the request is for a protected resource (@RolesAllowed etc.)
...
//If protected, check whether the request includes a valid token, (observe no need for a database lookup)
